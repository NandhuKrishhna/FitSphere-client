            ...
            data.notification.notification.map((notification: Notification) => (
              ...
              >
                ...
                <div>
                  ...
                  {!notification.read && <div className="w-4 h-4 bg-indigo-500 rounded-full ml-16 mt-1"></div>}
                </div>
              </div>
            ))}
        </div>
          ...
          <div className="h-[100%] bg-[#1E1E1E] rounded-lg overflow-hidden shadow-xl border border-[#2A2A2A] transition-all duration-300 flex-1">
            ...
            <div className="bg-[#2C2C2C] p-6 text-center">
              <div className="w-24 h-24 rounded-full border-4 border-[#3A3A3A] mx-auto mb-4 overflow-hidden flex items-center justify-center bg-gray-500">
                <span className="text-white text-2xl font-bold">
                  {selectedNotification.metadata?.name?.slice(0, 2).toUpperCase()}
                </span>
              </div>
              <h2 className="text-white text-2xl font-bold">{selectedNotification.metadata?.name}</h2>
            ...
            <div className="p-6 space-y-4 text-white">
              <div className="flex items-start space-x-3 flex-wrap">
                ...
                <p className="text-gray-300 flex-1">{selectedNotification.metadata?.bio}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{selectedNotification.metadata?.experience} years</p>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <Mail className="text-yellow-400" />
                  <div>
                    <p className="font- text-sm">{selectedNotification.metadata?.professionalEmail}</p>
                  </div>
                </div>
                {/*Gender*/}
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="font- text-sm">{selectedNotification.metadata?.gender}</p>
                  </div>
                </div>
                {/*Location*/}
                <div className="flex items-center space-x-2">
                  <Phone className="text-blue-400" />
                  <p className="text-gray-300">{selectedNotification.metadata?.contactPhoneNumber}</p>
                </div>
                {/*Location*/}
                <div className="flex items-center space-x-2">
                  <IndianRupee className="text-green-500" />
                  <p className="text-gray-300">{selectedNotification.metadata?.consultationFees}</p>
                </div>
                {/*Location*/}
                <div className="flex items-center space-x-2">
                  <MapPinHouse className="text-red-600" />
                  <p className="text-gray-300 text-sm">{selectedNotification.metadata?.officeAddress}</p>
                </div>
                {/*Location*/}
                <div className="flex items-center space-x-2">
                  <Locate />
                  <p className="text-gray-300 text-sm">{selectedNotification.metadata?.clinicLocations}</p>
                </div>
                {/*Location*/}
                <div className="flex items-center space-x-2">
                  <Languages />
                  <p className="text-gray-300">{selectedNotification.metadata?.consultationLanguages}</p>
                </div>
                {/*Location*/}
                <div className="flex items-center space-x-2">
                  <Album />
                  <p className="text-gray-300">{selectedNotification.metadata?.medicalLicenseNumber}</p>
                </div>
              </div>
              <button
                className="bg-zinc-600 inline-flex items-center  gap-3  justify-center rounded-sm text-white p-2 hover:bg-zinc-700 "
                onClick={() => {
                  setIsOpen(true);
                }}
              >
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="relative bg-indigo-400 p-4 rounded-lg shadow-lg">
            ...
            <img
              src={selectedNotification?.metadata?.profilePicture}
              alt="Attachment"
